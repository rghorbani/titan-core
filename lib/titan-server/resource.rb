module Titan
  module Server
    module Resource
      ...
      def wrap_resource(connection = Titan::Session.current)
        url = resource_url(:transaction)
        GremlinTransaction.new(url, connection)
      end
      ...
      def resource_headers
        {'Content-Type' => 'application/json', 'Accept' => 'application/json; charset=UTF-8', 'User-Agent' => ::Titan::Session.user_agent_string}
      end

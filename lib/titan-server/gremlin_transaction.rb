module Titan
  module Server
    # The GremlinTransaction object lifecycle is as follows:
    ...
    class GremlinTransaction
      include Titan::Transaction::Instance
      ...
      ROW_REST = %w(row REST)
      def _query(gremlin_query, params = nil)
        fail 'Transaction expired, unable to perform query' if expired?
        statement = {statement: gremlin_query, parameters: params, resultDataContents: ROW_REST}
        body = {statements: [statement]}

        response = @query_url ? query(body) : start(body)

        create_gremlin_response(response)
      end
      ...
      def create_gremlin_response(response)
        first_result = response.body[:results][0]

        GremlinResponse.new(response, true).tap do |gremlin_response|
          if response.body[:errors].empty?
            gremlin_response.set_data(first_result)
          else
            first_error = response.body[:errors].first
            mark_expired if first_error[:message].match(/Unrecognized transaction id/)
            gremlin_response.set_error(first_error)
          end
        end
      end

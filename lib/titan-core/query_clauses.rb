    ...
    module QueryClauses
      ...
      class WhereClause < Clause
        ...
        def hash_key_value_string(key, value, previous_keys)
          value.map do |k, v|
            if k.to_sym == :titan_id
              v = Array(v).map { |item| (item.respond_to?(:titan_id) ? item.titan_id : item).to_i }

module Titan
  ...
  # You can attach properties to relationships like Titan::Node.
  #
  ...
  #
  # Furthermore, Titan guarantees that a relationship is never "hanging freely,"
  ...
  class Relationship
    ...
    # @return [Titan::Node,Object] the node or wrapped node
    def start_node
      _start_node.wrapper
    end
    ...
    #
    # @param [Titan::Node] node the node that we don't want to return
    # @return [Titan::Node] the other node wrapper
    # @see #_other_node
    def other_node(node)
      _other_node(node.titan_obj).wrapper
    end
    ...
    class << self
      def create(rel_type, from_node, other_node, props = {})
        from_node.titan_obj.create_rel(rel_type, other_node, props)
      end

      def load(titan_id, session = Titan::Session.current)
        rel = _load(titan_id, session)
        rel && rel.wrapper
      end

      def _load(titan_id, session = Titan::Session.current)
        session.load_relationship(titan_id)
      end
    end
  end
end

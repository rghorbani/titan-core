...
describe Titan::Core::Query do
  describe 'options' do
    let(:query) { Titan::Core::Query.new(parser: 2.0) }

    it 'should generate a per-query gremlin parser version' do
      expect(query.to_gremlin).to eq('GREMLIN 2.0')
    end
    ...
    describe 'subsequent call' do
      ...
      it 'should combine the parser version with the rest of the query' do
        expect(query.to_gremlin).to eq('GREMLIN 2.0 MATCH q:Person')
      end
    end
  end
  ...
  describe 'batch finding' do
    ...
    before(:each) do
      Titan::Core::Query.new.match(foo: :Foo, bar: :Bar).delete(:foo, :bar).exec

      5.times do
        Titan::Node.create({uuid: SecureRandom.uuid}, :Foo)
      end
      2.times do
        Titan::Node.create({uuid: SecureRandom.uuid}, :Bar)
      end
    end
    ...
    describe 'find_in_batches' do
      {
        ...
        context "batch_size of #{batch_size}" do
          it "yields #{expected_yields} times" do
            expect do |block|
              Titan::Core::Query.new.match(f: :Foo).return(:f).find_in_batches(:f, :uuid, batch_size: batch_size, &block)
    ...
    describe 'find_each' do
      {
        ...
        context "batch_size of #{batch_size}" do
          it "yields #{expected_yields} times" do
            expect do |block|
              Titan::Core::Query.new.match(f: :Foo).return(:f).find_each(:f, :uuid, batch_size: 2, &block)
  ...
  describe 'DEFINED_CLAUSES' do
    it 'includes a key for each clause' do
      Titan::Core::Query::METHODS.each do |clause_string|
        expect(Titan::Core::Query::DEFINED_CLAUSES).to have_key(clause_string.to_sym)
      end
    end
  end
  ...
  describe '#clause?' do
    ...
    context 'checking for where' do
      ...
      context 'Query with a where' do
        let(:query) { Titan::Core::Query.new.where(true) }
        it { should be(true) }
      end

      context 'Query with an order' do
        let(:query) { Titan::Core::Query.new.order(:foo) }
        it { should be(false) }
      end

      context 'Query with a where and an order' do
        let(:query) { Titan::Core::Query.new.where(true).order(:foo) }
        it { should be(true) }
      end
    end
  end

  def expects_gremlin(gremlin, params = nil)
    query = eval("Titan::Core::Query.new#{self.class.description}") # rubocop:disable Lint/Eval
    expect(query.to_gremlin).to eq(gremlin)
    expect(query.send(:merge_params)).to eq(params) if params
  end

  def self.it_generates(gremlin, params = nil)
    it "generates #{gremlin}" do
      expects_gremlin(gremlin, params)
    end
  end
  ...
  describe 'merging queries' do
    let(:query1) { Titan::Core::Query.new.match(p: Person) }
    let(:query2) { Titan::Core::Query.new.match(c: :Car) }

    it 'Merging two matches' do
      expect((query1 & query2).to_gremlin).to eq('MATCH (p:`Person`), (c:`Car`)')
    end

    it 'Makes a query that allows further querying' do
      expect((query1 & query2).match('(p)-[:DRIVES]->(c)').to_gremlin).to eq('MATCH (p:`Person`), (c:`Car`), (p)-[:DRIVES]->(c)')
    end
  ...
  describe '#where' do
    ...
    describe '.where(q: {titan_id: 22})' do
  ...
  describe '#return' do
    ...
    describe '.return(q: :titan_id)' do
      it_generates 'RETURN ID(q)'
    end

    describe '.return(q: [:titan_id, :prop])' do
  ...
  describe '#union_gremlin' do
    it 'returns a gremlin string with the union of the callee and argument query strings' do
    ...
    it 'can represent UNION ALL with an option' do
      q = Titan::Core::Query.new.match(o: :Person).where(o: {age: 10})
      result = Titan::Core::Query.new.match(n: :Person).union_gremlin(q, all: true)
